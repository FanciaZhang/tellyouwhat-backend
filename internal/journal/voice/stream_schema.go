...
func streamSchema(raw []byte) []StreamSchema {
	...
	for _, u := range wire.Result.Utterances {
		...
		v := StreamSchema{Speaker: jsonKind(u["speaker"]), SpeakerID: jsonKind(u["speaker_id"]), AdditionSpeaker: jsonKind(a["speaker"]), AdditionSpeakerID: jsonKind(a["speaker_id"]), Volume: jsonKind(a["volume"]), SpeechRate: jsonKind(a["speech_rate"])}
