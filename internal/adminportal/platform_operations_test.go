...
func TestOperationsHTTPPublicationLifecycle(t *testing.T) {
	...
	policy, err := config.LoadOperationsDefaults()
	if err != nil {
		t.Fatal(err)
	}
	if err = store.Initialize(ctx, policy, now); err != nil {
		t.Fatal(err)
	}
	...
	root := "/api/v1/platform"
	for _, path := range []string{"/config", "/metrics", "/config/history", "/config/revisions/" + current.ID} {
	...
	check(res, 200)
	var draft platformops.Revision
	if err = json.Unmarshal(res.Body.Bytes(), &draft); err != nil {
		t.Fatal(err)
	}
	preview := func() string {
		t.Helper()
		res := call("GET", root+"/config/revisions/"+draft.ID, nil, true, false, "")
		check(res, 200)
		var out struct {
	...
	body := map[string]any{"revision": draft.ID, "baseVersion": current.ID, "previewToken": preview()}
	check(call("POST", root+"/config/publish", body, true, true, uuid.NewString()), 401)
	now = now.Add(6 * time.Minute)
	session.ReauthenticatedAt = now
	put()
	check(call("POST", root+"/config/publish", body, true, true, uuid.NewString()), 409)
	...
	session.ReauthenticatedAt = now
	put()
	replay := call("POST", root+"/config/publish", body, true, true, key)
	check(replay, 200)
	if replay.Body.String() != res.Body.String() {
		t.Fatal("publication replay changed result")
	}
	check(call("POST", root+"/config/drafts", input, true, true, uuid.NewString()), 409)
	var count int
	if err = db.QueryRow(`SELECT COUNT(*) FROM admin_audit_events WHERE action='operations.publish'`).Scan(&count); err != nil || count != 1 {
