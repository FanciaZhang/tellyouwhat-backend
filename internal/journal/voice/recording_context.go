...
passages 是正文与原始录音之间的可核验来源。无论正文是否需要 patch，都要为本次录音形成的每个最终正文段落返回一条 passages；不要给与本次录音无关的旧正文建立来源。blockID 是拆分前承载该段落的正文块 id，paragraphIndex 是该块按换行拆分、忽略空行后的从 0 开始序号。sourceIDs 必须按时间顺序原样复制该段内容依据的 utterance.sourceID；同一 passage 内必须是连续的 utterances，省去中间口头填充时也把中间 sourceID 纳入，保证用户回听的是连续原声。不得根据文字相似度猜来源。每条 passage 至少一个 sourceID。
情绪是正文旁的小符号，不是要写进正文的情绪描写。只能从 calm、happy、excited、relaxed、moved、hopeful、surprised、worried、nervous、sad、angry、tired 中选择。对每个有意义的声音情绪转折，最多返回 8 个 emotions；不要把每句话都标记。neutral 或 calm 只是没有明显转折，不需要单独标记；但只要 utterances 中存在非空且不是 neutral/calm 的 acousticEmotion，emotions 就不能全部省略，至少选择一个最有代表性的非中性声音证据放到对应内容附近。sourceID 必须原样复制有非空 acousticEmotion 的对应 utterance.sourceID；kind 是将该证据稳健映射到上述枚举，不凭文字臆测人的心理状态。blockID 必须是整理后承载对应内容的段落 id，且 sourceID 必须同时包含在该 blockID 的某条 passage 中；anchorText 必须是从该段落最终 text 中原样复制、且在该段中只出现一次的连续短语（最多 80 字），小符号会放在该短语之后。dialogue 模式可以从原样展开后的发言文字中选 anchorText；如果一个短语不唯一，要换用同段中更具体且唯一的短语，不能因此把所有非中性情绪都省略。overallEmotion 必须从同一枚举中选一个，表示整篇最显著的记录氛围；混合或证据不明时选 calm，不添加医学或心理结论。
只输出严格 JSON：{"baseRevision":整数,"transcriptRevision":整数,"patches":[{"id":"...","text":"...","afterID":""}],"passages":[{"blockID":"...","paragraphIndex":0,"sourceIDs":["utterance-source-id"]}],"questions":["..."],"emotions":[{"blockID":"...","anchorText":"...","sourceID":"...","kind":"happy"}],"overallEmotion":"calm"}。
...
passages 是最终正文与原始录音之间的可核验来源。即使某段正文无需 patch，也要为本次录音形成的每个最终正文段落返回一条 passage；不要给录音前已有的无关正文建立来源。blockID 是拆分前的正文块 id，paragraphIndex 是该块按换行拆分、忽略空行后的从 0 开始序号。sourceIDs 按时间顺序原样复制该段依据的连续 utterance.sourceID；即使整理时省去了中间填充，也要纳入中间 sourceID，以便连续回听。不得根据文字相似度猜来源，每条 passage 至少一个 sourceID。
...
sourceID 必须原样复制有非空 acousticEmotion 的对应 utterance.sourceID；kind 是将该证据稳健映射到上述枚举，不凭文字臆测人的心理状态。blockID 必须是当前承载对应内容的段落 id，且 sourceID 必须同时包含在该 blockID 的某条 passage 中；anchorText 必须从该段落当前 text 中原样复制、且在该段中只出现一次的连续短语（最多 80 字）。如果一个短语不唯一，换用同段更具体且唯一的短语。
