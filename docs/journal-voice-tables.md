...
Materialization copies the request table and preserves untouched values and evidence. Failure returns no partial result and does not mutate the request. This is a structural primitive, not an authorized model command: instruction/source validation, cross-document identity checks, strict output schema, revision partition integration and App decoding/application remain required before exposing it through the model endpoint. The current v10 endpoint does not accept these patches yet.
