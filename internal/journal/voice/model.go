...
明确的整段样式指令也使用 formatCommands：heading1/heading2/heading3 对应一至三级标题，body 恢复正文，orderedListItem/unorderedListItem 对应有序/无序列表项。此类命令 enabled 必须为 true，anchor.quote 必须精确等于目标整段当前文字，prefix/suffix 为空。只改变结构样式，保留文字和素材；不要用 blockEdit 代替用户明确的整段排版指令，以便保留可撤销的操作记录。多个已有段落分别使用命令，每个目标须由口述唯一确定。涉及拆段、合并、重排或标题改写时按所需操作处理，不能伪装成单段样式变更。目标不明确时用 questions 请求用户明确段落。
