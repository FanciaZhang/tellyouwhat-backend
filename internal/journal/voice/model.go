...
这不是整篇重写。pendingUtterances 只包含本轮新确认的口述；contextBlocks 只是有界的行文尾部。只整理新口述，保留第一人称、事实细节、感受和语气，删掉无意义口头重复，调整语法和局部衔接。不写会议要点，不压缩成摘要，不添加没说过的经历或推断。person 只在用户已经指定时才是人物身份；speaker 只是本次识别的声音线索，绝不得据此猜人。startMilliseconds、endMilliseconds、acousticEmotion、volume 和 speechRate 是该句口述的原始声学证据，不得改写或臆测。
...
每个有文字的 patch 都必须返回 passage：blockID 是 patch id，paragraphIndex 为 0，sourceIDs 按顺序列出该段使用的 pendingUtterances id。一个 sourceID 只能用一次。consumedSourceIDs 必须原样列出本轮所有 pendingUtterances id；即使某句只是需要丢弃的口头语，也要标记已处理，避免重复消耗。存在多种解释时保留原话并在 questions 提简短疑问。
...
func (m ArkRewriter) Rewrite(ctx context.Context, s Snapshot, tr int) (RewriteResult, error) {
	...
	emotionFields := map[string]any{"type": "object", "additionalProperties": false, "required": []string{"blockID", "anchorText", "sourceID", "kind"}, "properties": map[string]any{"blockID": map[string]string{"type": "string"}, "anchorText": map[string]string{"type": "string"}, "sourceID": map[string]string{"type": "string"}, "kind": map[string]any{"type": "string", "enum": emotionKinds}}}
	schema := map[string]any{"type": "object", "additionalProperties": false, "required": []string{"baseRevision", "transcriptRevision", "patches", "passages", "consumedSourceIDs", "questions", "emotions", "overallEmotion"}, "properties": map[string]any{"baseRevision": map[string]string{"type": "integer"}, "transcriptRevision": map[string]string{"type": "integer"}, "patches": map[string]any{"type": "array", "items": fields}, "passages": map[string]any{"type": "array", "items": passageFields}, "consumedSourceIDs": map[string]any{"type": "array", "items": map[string]string{"type": "string"}}, "questions": map[string]any{"type": "array", "items": map[string]string{"type": "string"}}, "emotions": map[string]any{"type": "array", "items": emotionFields}, "overallEmotion": map[string]any{"type": "string", "enum": append([]string{""}, emotionKinds...)}}}
