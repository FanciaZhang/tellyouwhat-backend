...
func (s Snapshot) Validate() error {
	...
	if s.incremental() {
		return s.validateIncremental()
	}
	return nil
}
func (r Revision) Validate(s Snapshot) error {
	if s.incremental() || r.BlockEdits != nil || r.Corrections != nil {
		if r.Patches != nil {
			return ErrInvalid
		}
		return r.validateIncremental(s)
	}
...
type Passage = PassageSource
type Emotion = EmotionPlacement

func (s Snapshot) validateIncremental() error {
	if s.Revision < 0 || len(s.Blocks) > 1024 || len(s.Words) > 32 || len(s.EditedBlockIDs) > 1024 ||
		...
		len(s.PendingUtterances) > MaxPendingUtterances || utf8.RuneCountInString(string(s.WritingStyle)) > 64 {
		return ErrInvalid
	}
	count := utf8.RuneCountInString(s.Transcript)
	seen := map[string]bool{}
...
func (r Revision) validateIncremental(s Snapshot) error {
