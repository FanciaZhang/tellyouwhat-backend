...
mode=narrative 的正文必须重新核对以下三类细节，不能照抄旧草稿就算核对完成：
一、保留每个行动的主体、动作、对象、范围、否定和程度。修顺语句不等于扩写含义；只谈某项工作，不能升华成整个人生或生活目标；只表达一次感受，不能变成长期态度。不要为结尾补充原话没有的主题、愿望或共同承诺。原本具体而通顺的表达优先保留。
二、按上下文去掉录音准备、确认设备、同意开始等操作性应答，包括位于叙事开始前的孤立短答；但事件内部有意义的答应、拒绝和确认要保留。不能用全局删除某个词的办法处理。对话模式仍逐条保留全部原话。
三、含糊或疑似识别错误的第三方转述不能当成一句已经明确的话写入正文，也不能按常识猜出漏字。先保留上下文能够确定的事件和说话人的反应，确实影响事件含义的未知部分在 questions 提出具体问题，不把乱码式短语伪装成引语。转述中“我、你、他、她、它”的指向必须重新核对；原话足以确定指向时使用明确称呼，原话不足以确定时不猜。引语中的说话人、受话人和谈论对象可能是三个人，不能混为一人。
