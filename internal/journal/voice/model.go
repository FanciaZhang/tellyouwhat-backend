...
func (m ArkRewriter) Rewrite(ctx context.Context, s Snapshot, tr int) (RewriteResult, error) {
	...
	schema := map[string]any{"type": "object", "additionalProperties": false, "required": []string{"baseRevision", "transcriptRevision", "patches", "questions", "emotions", "overallEmotion"}, "properties": map[string]any{
		...
		"patches": map[string]any{"type": "array", "items": fields}, "questions": map[string]any{"type": "array", "items": map[string]string{"type": "string"}},
