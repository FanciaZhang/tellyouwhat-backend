...
func readInsights(ctx context.Context, tx *sql.Tx, now time.Time) (Insights, error) {
 ...
 EXISTS(SELECT 1 FROM operations_purchase_observations p WHERE p.app_id=c.app_id AND p.key_id=c.key_id AND p.environment='production' AND p.purchased_at>=c.first_free_at AND p.purchased_at<=DATE_ADD(c.first_free_at,INTERVAL ? DAY) AND p.purchased_at<=? AND (p.price_milli IS NULL OR p.started_at IS NULL)) OR EXISTS(SELECT 1 FROM managed_entitlements e WHERE e.app_id=c.app_id AND e.key_id=c.key_id AND e.environment='production' AND e.price_milli IS NULL AND (e.started_at IS NULL OR (e.started_at>=c.first_free_at AND e.started_at<=DATE_ADD(c.first_free_at,INTERVAL ? DAY)))) AS price_unknown
 FROM operations_free_cohorts c WHERE c.first_free_at>=? AND c.first_free_at<=?)
 SELECT app_id,COUNT(*),SUM(NOT returning_subscriber),SUM(NOT returning_subscriber AND first_free_at<=?),SUM(NOT returning_subscriber AND first_free_at<=? AND paid),SUM(NOT returning_subscriber AND first_free_at>?),SUM(NOT returning_subscriber AND first_free_at<=? AND price_unknown) FROM candidates GROUP BY app_id`, days, now, days, now, days, out.CohortStart, now, now.AddDate(0, 0, -days), now.AddDate(0, 0, -days), now.AddDate(0, 0, -days), now.AddDate(0, 0, -days))
		if err != nil {
			return out, err
		}
		for rows.Next() {
